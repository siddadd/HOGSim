...
/*		Usage	       : HoG_Test_Vehicle.exe -i <ImageDirectory> -o <OutputDirectory> -m <0/1> -gt <GroundTruthDirectory>	*/
/****************************************************************************************************************************/

// HoG_Test_Vehicle.cpp : Defines the entry point for the console application.
// compile with: /EHsc /link hwHOGDll.lib

// Currently testing HOG-Pedro only - 01/11/2015 
...
int main(int argc, char* argv[])
{
	...
	if (argc < 3)
	{
		cerr << "[ERROR]: Usage is HoG_Test_Vehicle.exe " // command name
			<< " [-i <ImageDirectory>] [-o <OutputDirectory>] [-m <0/1>] [-gt <GroundTruthDirectory>]" << endl
			<< "Demo program to simulate hardware HOG" << endl
			<< " -i : input directory" << endl
			<< " -o : output directory" << endl
			<< " -m : 0 = software version (Dalal-OpenCV), 1 = hardware version (Pedro-default), 2 = hardware version (Dalal)" << endl
	...
	for (int fn = 0; fn < inputFiles.size(); fn++)
	{
		...
		// C Model // need to add case for hw-dalal
		IHOG *hwPHOG = new HardwarePedroHOG;

		int KernelWidth = KERNELWIDTH;
		int KernelHeight = KERNELHEIGHT;

		// (tunable) threshold to get bounding box 
		int dotPThreshold = DETECTIONTHRESHOLD;
		...
		if (runHardware)
		{
			...
			for (int layer = 0; layer < numLayer; layer++)
			{
				...
				Mat inputImage = scaledImage(Rect(0, 0, imageSize.width, imageSize.height));
				...
				__int32* bins = hwPHOG->RHA(inputImage);
				...
				loadDescriptorVectorFromFile(hmdescriptorVectorFile.c_str(), svmData);
				Mat svmRoIData = svmData.reshape(SVMROICHANNELS, SVMROIWIDTH);
				hwPHOG->svm = convertSVM(svmRoIData);

				/**************************/
				// { // C MODEL - NCA
				/**************************/

				dotP = hwPHOG->NCA(bins, roi_pix_height, roi_pix_width, 1);
				...
				displayDotP(inputImage, dotP, DETECTIONTHRESHOLD);
#endif
		...
		else
		{
			...
			vector <float> ocDetector = swDHOG.getDefaultPeopleDetector();
...
#endif
		// Write to Output
		wstring outDirWintemp = wstring(outDirectory.begin(), outDirectory.end());
		LPCWSTR outDirWin = outDirWintemp.c_str();
	...
	*/
	void loadDescriptorVectorFromFile(const char* fileName, Mat& descriptor)
	{
		...
		const Mat tmpdescriptor = mlData->get_values();

		// Need to do physical copy to new location
		tmpdescriptor.copyTo(descriptor);

		//cout << descriptor.at<float>(0, 0) << endl; 
		//cout << tmpdescriptor.at<float>(0, 0) << endl;
